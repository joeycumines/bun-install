...
/**
 * Entry point: discovers the caller's workspace, prunes the graph to the
 * requested commands, builds and packs the required packages, then installs
 * them globally and verifies the resulting binaries.
 */
function main(): void {
  ...
  if (commandToPackage.size === 0) {
    die(
      'No commands (binaries) found in any workspace packages. Nothing to install.',
    );
  }
