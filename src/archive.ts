...
export function resolveArchiveStoreDir(projectId: string): string {
  ...
  if (process.platform === 'win32') {
    ...
    if (!localAppData) {
      die(
        'LOCALAPPDATA is not set on Windows; cannot determine a persistent store directory.',
      );
    }
  ...
  if (process.platform === 'darwin') {
    ...
    if (!home) {
      die(
        'Neither HOME nor USERPROFILE is set; cannot determine a persistent store directory.',
      );
    }
...
export function moveIntoStore(srcPath: string, destPath: string): void {
  ...
  if (process.platform === 'win32') {
    ...
    try {
      ...
      console.warn(
        'Atomic rename failed on Windows, falling back to copy + remove',
      );
    }
    ...
    try {
      ...
      console.warn(
        'Could not remove source after cross-device copy; temp cleanup will sweep it',
      );
    }
    return;
  }
